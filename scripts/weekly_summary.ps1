param(
    ...
    [switch]$All,
    [string]$WeeklyHistoryCsvPath = ".\state\weekly_summary_history.csv"
)
...
Write-Host ""
Write-Host "=== Weekly Performance Summary ===" -ForegroundColor Green
if ($All) {
...
$weeklyHistoryDir = Split-Path -Parent $WeeklyHistoryCsvPath
if ($weeklyHistoryDir -and -not (Test-Path $weeklyHistoryDir)) {
    New-Item -ItemType Directory -Path $weeklyHistoryDir -Force | Out-Null
}

$windowMode = if ($All) { "ALL" } else { "LOOKBACK_DAYS" }
$weeklyRow = [pscustomobject]@{
...
$weeklyAppend = Test-Path $WeeklyHistoryCsvPath
$weeklyRow | Export-Csv -Path $WeeklyHistoryCsvPath -NoTypeInformation -Append:$weeklyAppend
Write-Host ("Weekly History CSV: {0}" -f $WeeklyHistoryCsvPath) -ForegroundColor DarkGray
