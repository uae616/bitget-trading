...
class ConvertAPI:
    ...
    def exchange_info(self):
        return self.client.signed_get('/sapi/v1/convert/exchangeInfo')

    def asset_info(self):
        return self.client.signed_get('/sapi/v1/convert/assetInfo')
    ...
    def convert_buy_from_usdt(self, coin: str, usdt_amount: float, timeout=5.0):
        quote = self.get_quote(from_asset='USDT', to_asset=coin, from_amount=usdt_amount)
        quote_id = quote.get('quoteId')
        if not quote_id:
            return None
