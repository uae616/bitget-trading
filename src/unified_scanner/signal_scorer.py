...
def _htf_series(closes: list[float], factor: int) -> pd.Series:
    if factor <= 1:
        return pd.Series(closes, dtype=float)
    s = pd.Series(closes, dtype=float)
    grouped = s.groupby(np.arange(len(s)) // factor).last()
...
def score_signal(
    ...
    # ================================================================
    adx_val = adx(highs, lows, closes, int(_cfg(cfg, "adx_period")))
    if _cfg(cfg, "enable_adx_filter"):
        max_score += 1
        adx_min = float(_cfg(cfg, "adx_min"))
        if adx_val >= adx_min:
            if buy_score >= sell_score:
                buy_score += 1
            else:
                sell_score += 1
            reasons.append(f"ADX confirms trend ({adx_val:.1f} >= {adx_min:.1f})")
        else:
            reasons.append(f"ADX weak trend ({adx_val:.1f} < {adx_min:.1f})")
            buy_score = max(0, buy_score - 1)
            sell_score = max(0, sell_score - 1)

    # ================================================================
    # 12. HTF bias filter (downsampled close series)
    # ================================================================
    htf_ma = 0.0
    if _cfg(cfg, "enable_htf_filter"):
        htf_factor = int(_cfg(cfg, "htf_factor"))
        htf_len = int(_cfg(cfg, "htf_ma_len"))
        htf_closes = _htf_series(closes, htf_factor)
        htf_ma = sma(htf_closes, htf_len) if len(htf_closes) >= htf_len else 0.0
        max_score += 1
        if htf_ma > 0:
            if last_close > htf_ma:
                buy_score += 1
                reasons.append("HTF bias bullish (price above HTF SMA)")
            else:
                sell_score += 1
                reasons.append("HTF bias bearish (price below HTF SMA)")
